...
namespace IAViewer.WebCrawl.Core
{
    public class TaskThreadManager : IThreadManager
    {
        ...
        /// Note that 1 database does not equal 1 thread.
        /// </summary>
        ...
        /// <summary>
        /// Wait for a database to become available and then perform the specified action.
        /// </summary>
        public void DoWork(Action action)
        {
            ...
            //Spin until we can create a new database without exceeding the limit
            while (true)
            {
                if (_freeTaskCount > 0)
                {
            ...
            _logger.DebugFormat("Starting up a database on thread id {0}.", Thread.CurrentThread.ManagedThreadId);
